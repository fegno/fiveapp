...
class UserInviteModule(APIView):
    ...
    def post(self, request):
        ...
        if admin_login.user_type == 'ADMIN':
            ...
            # #     response_dict["error"] = "User already exists"
            # #     return Response(response_dict, status=status.HTTP_200_OK)
            # availble_free_user = user_login.available_free_users
            # available_paid_user = user_login.available_paid_users
            ...
            #                 assigned_module_names.append(module.title)
            #             deleted_user.save()
            #             user_login.available_free_users -= 1
            #             user_login.save()
            #     else:
            #         if available_paid_user>0:
            #             if deleted_user:
            #                 if deleted_user.is_free_user == False:
            #                     deleted_user.is_active = True
            ...
            #                         assigned_module_names.append(module.title)
            #                     deleted_user.save()
            #                     user_login.available_paid_users -= 1
            #                     user_login.save()
            #         else:
            #                 response_dict["error"] = "No paid or free users are available"  
            #                 return Response(response_dict, status=status.HTTP_200_OK)
